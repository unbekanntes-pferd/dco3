...
use crate::{
    ...
    constants::{DRACOON_API_PREFIX, USER_ACCOUNT, USER_ACCOUNT_CUSTOMER, USER_BASE},
    utils::FromResponse,
};

use super::{
    models::{UpdateUserAccountRequest, UserAccount},
    CustomerData, User, UserEndpoint,
};
...
impl User for UserEndpoint<Connected> {
    ...
    async fn get_customer_info(&self) -> Result<CustomerData, DracoonClientError> {
        let url_part =
            format!("{DRACOON_API_PREFIX}/{USER_BASE}/{USER_ACCOUNT}/{USER_ACCOUNT_CUSTOMER}");
        ...
        CustomerData::from_response(response).await
    }
}
